...
public:
	...
	bool Save(std::wstring sFile)
	{
		...
		fwrite(m_Colours, sizeof(short), (int)nWidth * nHeight, f);
		fwrite(m_Glyphs, sizeof(short), (int)nWidth * nHeight, f);
	...
	bool Load(std::wstring sFile)
	{
		...
		std::fread(m_Colours, sizeof(short), (int)nWidth * nHeight, f);
		std::fread(m_Glyphs, sizeof(short), (int)nWidth * nHeight, f);
...
public:
	...
	int ConstructConsole(int width, int height, int fontw, int fonth)
	{
		...
		m_bufScreen = new CHAR_INFO[(int)m_nScreenWidth * (int)m_nScreenHeight];
	...
	void DrawString(int x, int y, std::wstring c, short col = 0x000F)
	{
		...
		{
			m_bufScreen[y * (int)m_nScreenWidth + x + i].Char.UnicodeChar = c[i];
			m_bufScreen[y * (int)m_nScreenWidth + x + i].Attributes = col;
		}
	}
	...
	void GameThread()
	{
		...
		while (m_bAtomActive)
		{
			...
			while (m_bAtomActive)
			{
				...
				wchar_t s[256];
				swprintf_s(s, 256, L"Andreas Console Game Engine - %s - FPS: %3.2f", m_sAppName.c_str(), 1.0f / fElapsedTime);
				SetConsoleTitle(s);
...
protected: // Audio Engine =====================================================================
		...
		unsigned int nBlocks = 8, unsigned int nBlockSamples = 512)
	{
		...
		for (unsigned int n = 0; n < m_nBlockCount; n++)
		{
			...
			m_pWaveHeaders[n].lpData = (LPSTR)(m_pBlockMemory + (n * (int)m_nBlockSamples));
		}
